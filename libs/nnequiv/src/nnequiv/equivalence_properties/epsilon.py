    ...
    def check(self, zono: ZonoState, use_exact=False):
        ...
        if eps > self.epsilon:
            ...
            if too_high.any():
                ...
                pos2 = 0
            init_bounds_nparray = np.array(
                output_zonos[1].init_bounds, dtype=zono.zono.dtype
            )
            if pos2 == 0:
                # Return lower bound
                rv = np.where(
                    out.mat_t[pos1] <= 0,
                    init_bounds_nparray[:, 1],
                    init_bounds_nparray[:, 0],
                )
            else:
                # Return upper bound
                rv = np.where(
    ...
    def fallback_check(self, zono, use_exact=False):
        ...
        for i in range(mat.shape[0]):
            try:
                ...
                raise UnsatError
            min_val = self.compute_deviation(
                min_vec,
                bias[i],
                mat[i],
                output_zonos[-1].init_bounds,
                0,
            ...
            try:
                ...
                Timers.toc("check_epsilon_fallback")
                raise UnsatError
            max_val = self.compute_deviation(
                max_vec,
                bias[i],
                mat[i],
                output_zonos[-1].init_bounds,
