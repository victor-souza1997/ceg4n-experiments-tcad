...
from nnenum.network import (AddLayer, FlattenLayer, FullyConnectedLayer,
                            MatMulLayer, ReluLayer)
...
class LpStarState(Freezable):
    ...
    def __init__(self, uncompressed_init_box=None, spec=None, safe_spec_list=None):
        ...
        if uncompressed_init_box is not None:
            assert isinstance(
                uncompressed_init_box, np.ndarray
            ), "init bounds should be given in a numpy array"
            assert uncompressed_init_box.dtype in [
                np.float32,
                np.float64,
    ...
    def from_init_box(self, uncompressed_init_box):
        ...
        self.prefilter = Prefilter()
        self.prefilter.init_from_uncompressed_box(
            uncompressed_init_box, self.star, init_box
        )
    ...
    def split_enumerate(self, i, network, spec, start_time):
        ...
        if not child_feasible:
            ...
            depth = len(self.branch_tuples)
            child.prefilter = self.prefilter.split_relu(
                i, pos.star, neg.star, self_gets_positive, start_time, depth
            )
