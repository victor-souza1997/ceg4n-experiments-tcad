...
class AgenState(Freezable):
    ...
    def __init__(self, onnx_filename, orig_image, label, epsilon, bounds=(0.0, 1.0)):
        ...
        with self.sess.as_default():
            ...
            self.fmodel = fb.models.TensorFlowModel(
                input_tensor, output_tensor, bounds=self.bounds
            )
    ...
    def try_mixed_adversarial(self, iteration, random_only):
        ...
        criterion = (
            fb.criteria.Misclassification() if t is None else fb.criteria.TargetClass(t)
        )

        with self.sess.as_default():
            if attack_class is None:
                attack_class = SingleEpsilonRPGD
                attack = SingleEpsilonRPGD(
                    self.fmodel, distance=fb.distances.Linfinity, criterion=criterion
                )
        ...
        if rv is not None:
            print(
                f"try_mixed_adversarial found violation image on iteration {iteration} with ep={dist} and "
        ...
        with self.sess.as_default():
            for attack_class in classes:
                ...
                if a.distance.value != np.inf:
                    ...
                    status = (
                        f"({factor}) {round(diff * 1000, 1)}ms, {round(a.distance.value, 5)} from "
...
def try_quick_adversarial(num_attempts, remaining_secs=None):
    ...
    if a is not None:
        ...
        if Settings.PRINT_OUTPUT and num_attempts > 0:
            print(
                f"try_quick_adversarial found violation image on iteration {i} with ep={ep}"
            )
