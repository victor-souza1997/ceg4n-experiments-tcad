...
def check_openblas_threads():
    ...
    for entry in d:
        assert entry["num_threads"] == 1, (
            "expected single thread libraries (export OPENBLAS_NUM_THREADS=1 and "
    ...
    def __setattr__(self, key, value):
        if self._frozen and not hasattr(self, key):
            raise AttributeError(
                "{} does not contain attribute '{}' (object was frozen)".format(
                    self, key
                )
            )
...
class FreezableMeta(type):
    ...
    def __setattr__(cls, key, value):
        if key != "_FROZEN" and cls._FROZEN and not hasattr(cls, key):
            raise AttributeError(
                f"type object '{cls.__name__}' has no attribute '{key}' (object was frozen)"
            )
...
def assert_verts_equals(verts, check_list, tol=1e-5):
    ...
    for v in check_list:
        assert pt_almost_in(v, verts, tol), "{} was not found in verts: {}".format(
            v, verts
        )
    ...
    for v in verts:
        assert pt_almost_in(
            v, check_list, tol
        ), "verts contains {}, which was not in check_list: {}".format(v, check_list)
...
def compress_init_box(input_box, tol=1e-9):
    ...
    assert dtype in [
        ...
        np.float64,
        np.float32,
    ], f"input_box dtype should be float32/64, got {dtype}"
