...
def worker_func(param):
    ...
    if lb < -Settings.SPLIT_TOLERANCE and ub > Settings.SPLIT_TOLERANCE:
        if (
            both_bounds
            or (ub_first and lb == -np.inf)
            or (not ub_first and ub == np.inf)
        ):
            ...
            assert (
                lb - 1e-1 < sim_i < ub + 1e-1
            ), f"sim[{i}]={sim_i} was not between bounds: {lb, ub}"

    return (i, lb, ub, num_lps)


def update_bounds_lp(
    layer_bounds,
    star,
    sim,
    split_indices,
    ...
    if n <= 1:
        rv = update_bounds_lp_serial(
            layer_bounds, star, sim, split_indices, check_cancel_func, both_bounds
        )
    else:
        rv = update_bounds_lp_parallel(
            layer_bounds, star, sim, split_indices, n, check_cancel_func, both_bounds
        )

    return rv


def update_bounds_lp_parallel(
    layer_bounds,
    star,
    sim,
    split_indices,
...
def update_bounds_lp_serial(
    layer_bounds, star, sim, split_indices, check_cancel_func=None, both_bounds=False
):
    """
    ...
    for index_in_list, i in enumerate(split_indices):
        ...
        if (
            ...
            assert (
                lb - 1e-1 < sim[i] < ub + 1e-1
            ), f"sim[{i}]={sim[i]} was not between bounds: {lb, ub}"
