...
class StateManager:
    ...
    def check(self, el: EnumerationStackElement):
        ...
        assert el.state.active
        try:
            equiv, data = self.eq_property.check(el.state)
        except UnsatError:
            valid = False
        ...
        if not valid and self.eq_property.allows_fallback(el.state):
            try:
                equiv, data = self.eq_property.fallback_check(el.state)
                valid, result = self.valid_result(el, equiv, data)
            except UnsatError:
                valid = False
                if not el.state.admits_refinement():
        ...
        if not valid and not el.state.admits_refinement():
            try:
                equiv, data = self.eq_property.check(el.state, use_exact=True)
                valid, result = self.valid_result(el, equiv, data)
            except UnsatError:
                valid = False
                if not el.state.admits_refinement():
