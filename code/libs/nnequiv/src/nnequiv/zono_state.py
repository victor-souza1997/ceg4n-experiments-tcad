...
"""
OverapproxPoint = namedtuple(
    "OverapproxPoint", ["network", "layer", "index", "coefficient"]
)
    ...
    def process_layer(self, zono, lpi, start_with=0):
        if self.output_bounds is None or start_with != 0:
            ...
            if self.branching_neurons is not None:
                new_zeros = (
                    self.branching_neurons[
                        self.output_bounds[self.branching_neurons, 1]
                        < -Settings.SPLIT_TOLERANCE
                    ]
                ).tolist()
            ...
            branching_neurons = (
                np.nonzero(
                    np.logical_and(
                        self.output_bounds[start_with:, 0] < -Settings.SPLIT_TOLERANCE,
            ...
            for branch_index in branching_neurons:
                ...
                if (
                    minVal < -zono.center[branch_index]
                    and maxVal > -zono.center[branch_index]
                ):
...
class ZonoState:
    ...
    def contract_domain(self, row, bias, index, networks, overflow):
        ...
        zeros = self.layer_bounds.process_layer(
            self.zono, self.lpi, start_with=index + 1
        )
            ...
            if self.do_branching is not None and cur_split_point <= self.do_branching:
                ...
                if (
                    ...
                    if self.before_overapprox is None:
                        self.before_overapprox = ZonoState(
                            self.network_count, state=self
                        )
    ...
    def overapproximate(self, index, networks: [NeuralNetwork]):
        ...
        self.overapprox_nodes.append(
            OverapproxPoint(self.cur_network, self.cur_layer, index, dim)
        )
    ...
    def do_first_relu_split(self, networks: [NeuralNetwork]):
        ...
        if pos is not None:
            pos.contract_domain(
                -row, bias, index, networks, self.layer_bounds.output_bounds[index, 1]
            )
            pos.branching.append(
                SplitPoint(split_net, split_layer, index, SplitDecision.POS)
            )
        if neg is not None:
            neg.contract_domain(
                row, -bias, index, networks, -self.layer_bounds.output_bounds[index, 0]
            )
            if neg.active:
                ...
                neg.branching.append(
                    SplitPoint(split_net, split_layer, index, SplitDecision.NEG)
                )
            else:
                rv = None
    ...
    def is_finished(self, networks: [NeuralNetwork]):
        ...
        if not self.active:
            ...
            return True
        if self.cur_network < self.network_count and self.cur_layer >= len(
            networks[self.cur_network].layers
        ):
            self.cur_layer = 0
            ...
            new_zono = Zonotope(
                init_center, init_mat, init_bounds=self.zono.init_bounds
            )
    ...
    def get_output_zonos(self):
        ...
        for i in range(len(self.output_zonos) - 1):
            ...
            if missing_dims > 0:
                mat_t = np.pad(self.output_zonos[i].mat_t, ((0, 0), (0, missing_dims)))
                rv.append(
                    Zonotope(
                        self.output_zonos[i].center,
                        mat_t,
                        self.output_zonos[i].init_bounds,
                    )
                )
            else:
...
def status_update():
    ...
    if GLOBAL_STATE.FINISHED_FRAC > 0:
        ...
        print(
            f"\rRefined: {GLOBAL_STATE.REFINED} | Refine Limit: {GLOBAL_STATE.REFINE_LIMIT} | Wrong: {GLOBAL_STATE.WRONG} | Right: {GLOBAL_STATE.RIGHT} | Total: {total} | Total Zonos Considered: {total + GLOBAL_STATE.REFINED} | Expected {expected} ({percentage}%)",
            end="",
        )
